      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white/80 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-blue-100 hover:scale-105 transition-all duration-300">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent uppercase tracking-wider">Utilisateurs</p>
        ...
        <div className="bg-white/80 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-purple-100 hover:scale-105 transition-all duration-300">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent uppercase tracking-wider">Points d'intérêt</p>
              <h3 className="text-3xl font-bold text-gray-800 mt-2">{stats?.totalPois || 0}</h3>
            </div>
            ...
            {stats?.poisByCategory && Object.entries(stats.poisByCategory).map(([category, count], idx) => (
              <div key={category}>
                ...
                <div className="w-full bg-gray-100 rounded-full h-2.5">
                  <div 
                    className={`h-2.5 rounded-full ${['bg-blue-500', 'bg-purple-500', 'bg-green-500', 'bg-yellow-500', 'bg-red-500'][idx % 5]}`} 
                    style={{ width: `${(count / stats.totalPois) * 100}%` }}
                  ></div>
                </div>
              </div>
            ))}
            {(!stats?.poisByCategory || Object.keys(stats.poisByCategory).length === 0) && (
              <p className="text-gray-400 text-center py-8">Aucune donnée de catégorie disponible</p>
            )}
          </div>
        </div>
